...
class _Line:
    def __init__(self, linestart, linestop):
        ...
        try:
            ...
            self.zero_y = None
    def __str__(self):
        return str(self.tolist()[0])+"->"+str(self.tolist()[1])
...
class _NewPolygon:
    def __init__(self, xypoints=None):
        ...
        """
        if xypoints:
            #loop and connect nodes
            self.first = _Node(*xypoints[0])
            previous = self.first
            for xy in xypoints[1:]:
                node = _Node(*xy)
                node.prev = previous
                node.prev.next = node
                previous = node
    ...
    def InsertNode(self, node, prev, next):
        """
        node is a basic node with only alpha information, this function will take care of its prev and next nodes
        prev and next are the normal nodes on its sides
        """
        _searchnode = prev
...
def _pointinring(point, ring): 
    # By: Karim Bahgat
    inside = False
    lastpolypoint = ring.last
    for polypoint in ring:
...
def _phase1(subjpoly, clippoly):
    ...
    """
    subjpoly = _NewPolygon(subjpoly)
    clippoly = _NewPolygon(clippoly)
    ###
    ...
    for subjnode in itersubjpoly:
        ...
        for clipnode in iterclippoly:
            ...
            if intersection:
                ...
                #first in subjpoly
                intsecline = _Line(subjnode,intersection)
