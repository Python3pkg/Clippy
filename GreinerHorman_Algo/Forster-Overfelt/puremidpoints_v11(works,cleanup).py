...
##                        #both subj and clip intersect each other somewhere in the middle
                    ...
                    s_intsecs.append( (iS, s, self.next(s.__next__)) )
                    c_intsecs.append( (iC, c, clip.next(c.__next__)) )
...
##will take ...
            ...
            if s.intersect:
                ...
                if prevloc == "out":
                    ...
                    if nextloc == "on":
                        ...
                        s.entry = not s_entry
                        print("maybe fix", intersectionmode, preventry, s.entry)
                        if intersectionmode and not preventry and not s.entry:
                            print("fixin")
                            # prevent going back and forth
                            s.intersect = False
                            ss = s.prev
                            while not ss.intersect:
                                ss = ss.prev
                            ss.intersect = False
                    elif nextloc == "in":
                        s.entry = s_entry
            ...
            if c.intersect:
                ...
                if prevloc == "out":
                    ...
                    if nextloc == "on":
                        ...
                        c.entry = not c_entry
                        if intersectionmode and not preventry and not c.entry:
                            # prevent going back and forth
                            c.intersect = False
                            cc = c.prev
                            while not cc.intersect:
                                cc = cc.prev
                            cc.intersect = False
                    elif nextloc == "in":
                        c.entry = c_entry
        ...
        # phase two and a half - all intersections between subject and clip have been deleted,
        ...
        # --------------------------------------------------
        resultpolys = []
        current = next_unprocessed(self.first)
        while current:
            ...
            while True:
                current.setChecked()
                ##print "prechecked",current,current.checked,current.neighbour.checked
                    ...
                    while True:
                        ...
                        if current.intersect:
                            if current.entry == preventry:
                                current.setChecked()
                            #elif intersectionmode and preventry and not current.entry:
                            #    current.setChecked()
                ...
                ##print "none test", current, current.neighbour
            ...
            # ........
            # ........
            #print "add poly?",clipped.first.next.next != clipped.first, clipped.first == clipped.first.prev

            # to avoid back and forth intersection when just touching using: clipped.first.next.next.xy != clipped.first.xy
            # however, that assumes only one two touching degens, but could be multiple touching degens
            # so need better testing for this special case...
            if clipped.first.xy == clipped.first.prev.xy:
                polytuple = (clipped, [])
                resultpolys.append(polytuple)

            #print "last?", current, self.first, self.first.neighbour
            if current in (self.first,self.first.neighbour):
                break
