...
def next_unprocessed(vert):
    ...
    while vert:
        ...
        if vert.intersect and not vert.checked and not vert.neighbour.checked:
            # both neighbours have to be unprocessed to be considered unprocessed
            return vert
    ...
    def clip(self, clip, s_entry, c_entry):
        ...
        for s in self.iter(): # for each vertex Si of subject polygon do
            for c in clip.iter(): # for each vertex Cj of clip polygon do
                try:
                    print("find isect %s - %s and %s - %s" %(s.xy, self.next(s.__next__).xy, c.xy, clip.next(c.__next__).xy ))
            ...
            if s.intersect:
                ...
                if prevloc == "out":
                    ...
                    if nextloc == "in":
                        ...
                        s.entry = not s_entry
                        # no interest in entry tangents
                        # ...though still needs entry flag
                        s.intersect = False
                elif prevloc == "on":
                    if nextloc == "on":
                        s.intersect = False
...
##                                s.intersect = False
        ...
        while current:
            ...
            while True:
                ...
                if current.entry:
                    while True:
                        ...
                        print("next",current)
                        clipped.add(Vertex(current))
                        if current.intersect or current.checked:
            ...
            current = next_unprocessed(current)
